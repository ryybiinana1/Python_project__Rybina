    ...
    async def _save_image(self, session: ClientSession, url: str) -> None:
        try:
            ...
            if response.status == HTTPStatus.OK:
                ...
                logging.info(f'Successful image saving {url}')
            else:
                logging.error(
                    f'Error when working with the image {response.status}'
                    )
        except Exception as e:
            logging.exception(
                f"Error loading {url}: {response.status} {e}"
                )
